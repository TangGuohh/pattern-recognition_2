...
import numpy as np
import os
img_dir = "C:\\pythonProject0N\\fruit_dataset\\"
...
def display_all(max_similar_num = 5):
    ...
    for i in range(num_png):
        ...
        arg_index  = np.argsort(result_list[i])[-(count+1):-1]
        print("第{}张图片相似图片索引：{}".format(i+1,arg_index))
        for j in arg_index:
            k = j + 1
            dir = img_dir + str(k) + ".jpg"
            print("和第{}张图片第{}相似的是第{}张图片||相似度:{}||路径:{}".format(i+1,count,k,result_list[i][j],dir))
            count -= 1
            cv2_img = cv2.imread(dir,1)
...
def display(index,max_similar_num):
    ...
    for j in arg_index:
        k = j + 1
        dir = img_dir + str(k) + ".jpg"
        print("和第{}张图片第{}相似的是第{}张图片||相似度:{}||路径:{}".format(index,max_similar_num,k,result_list[i][j],dir))
        max_similar_num -= 1
        cv2_img = cv2.imread(dir,1)
        cv2.imshow("第{}张相似图片".format(k),cv2_img)
        cv2.waitKey(0)
